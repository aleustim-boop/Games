...
(async()=>{const browser=await chromium.launch();try{
  ...
  await p.locator('#кат-доп-настройки').getByRole('checkbox',{name:/Дружелюбный/}).check();await p.locator('#кат-доп-настройки').getByRole('checkbox',{name:/Мягкий/}).check();await p.locator('#кат-доп-настройки').getByRole('combobox',{name:'Время на ход'}).selectOption('60');await p.getByRole('combobox',{name:'Гавани',exact:true}).selectOption('fixed');
  ...
  let record=await p.evaluate(()=>JSON.parse(localStorage.getItem('catan-match-v1')));assert.deepEqual(record.options,{friendlyRobber:true,easyStart:true,turnSeconds:60,targetPoints:12,harbors:'fixed'});
  ...
  await p.locator('#кат-фильтр').click();await p.getByRole('button',{name:'Мои земли',exact:true}).click();await p.getByRole('button',{name:'Готово',exact:true}).click();assert.equal(await p.locator('.кат-приглушён').count(),0,'Фильтр не должен прятать доступные цели');
