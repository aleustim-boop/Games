...
(async () => {
  ...
  console.log('\n=== 1. Ноты по событиям доски ===');
  {
    ...
    /* Доска теперь не первая: страница открывается на новом лобби
       ...
       повиснет на невидимом элементе). Кнопка «Вдвоём на одном
       телефоне» решением владельца 24.09 вернулась на ПЕРВЫЙ шаг лобби
       (id «лобби-вдвоём», сразу под тройкой карточек) — лобби уже
       открыто, жмём её без захода на второй шаг «Играть с ботом». */
  ...
  console.log('\n=== 2. Кнопка «Звук» в листе: выключение, память, включение ===');
  {
    ...
    // Страница открывается на лобби, доску открывает кнопка «лобби-вдвоём»
    // прямо на первом шаге (решение владельца 24.09).
    await страница.click('#лобби-вдвоём');
    await страница.waitForTimeout(300);
    ...
    await страница.waitForTimeout(200);
    await страница.click('#лобби-вдвоём');   // после перезагрузки снова начинаем с лобби, кнопка на первом шаге
    await страница.waitForTimeout(300);
    await страница.click('#кнопка-игра-ещё');
    await страница.waitForTimeout(250);
    проверить(await страница.evaluate(() =>
