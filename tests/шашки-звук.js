...
(async () => {
  ...
  console.log('\n=== 1. Ноты по событиям доски ===');
  {
    ...
    /* Доска теперь не первая: страница открывается на новом лобби
       ...
       повиснет на невидимом элементе). Отдельной карточки «Вдвоём» на
       первом шаге больше нет (правка 23.09) — «Вдвоём на этом
       устройстве» переехала на второй шаг «Играть с ботом»: открываем
       его карточкой «лобби-боты» и жмём там «кнопка-бота-вдвоём». */
    ...
    /* «Другой соперник» уводит на лобби — уровень бота выбирают на
       ВТОРОМ ШАГЕ: карточка «лобби-боты» его открывает, там жмём
       настоящую кнопку «уровень-сложный» и «Начать» (#кнопка-с-ботом)
       (см. js/шашки-лобби.js). Отдельная кнопка «кнопка-режим-бот»
       лежит теперь на запасном, спрятанном экране — жать её напрямую
       нельзя. */
    await страница.click('#лобби-боты');
    await страница.waitForTimeout(200);
  ...
  console.log('\n=== 2. Кнопка «Звук» в листе: выключение, память, включение ===');
  {
    ...
    // Страница открывается на лобби, доску открывает «кнопка-бота-вдвоём»
    // со второго шага «Играть с ботом» (первый шаг «Вдвоём» не несёт).
    await страница.click('#лобби-боты');
    await страница.waitForTimeout(200);
    await страница.click('#кнопка-бота-вдвоём');
    await страница.waitForTimeout(300);
    ...
    await страница.waitForTimeout(200);
    await страница.click('#лобби-боты');     // после перезагрузки снова начинаем с лобби
    await страница.waitForTimeout(200);
    await страница.click('#кнопка-бота-вдвоём');
    await страница.waitForTimeout(300);
    await страница.click('#кнопка-игра-ещё');
