...
(async()=>{
  ...
  try{
    const options={friendlyRobber:true,easyStart:true,turnSeconds:120};
    ...
    while(steps++<2000){
      ...
      assert.deepEqual(v.options,options);
      ...
      if(steps%25===0)console.log('Живой сервер: '+steps+' действий');
    }
    assert(steps<2000);console.log('Проверка опубликованного Катана — OK');
  }finally{
    const cleanup=await Promise.allSettled(players.map(p=>req('выйти',p)));
    for(const result of cleanup)if(result.status==='rejected')console.error('Проверочный участник: не удалось выйти —',result.reason.message);
  }
