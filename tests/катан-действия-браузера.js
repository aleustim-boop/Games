...
async function ход(page,v,a){
  ...
  if(['road','settlement','city'].includes(a.type)){
    if(v.phase==='main')await page.locator('#кат-стройки').getByRole('button',{name:new RegExp('^'+names[a.type]+'$')}).click();
  ...
  else if(a.type==='development')await page.locator('#кат-стройки').getByRole('button',{name:'Развитие',exact:true}).click();
  else if(a.type==='bank'){
