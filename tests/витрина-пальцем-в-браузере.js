...
(async function () {
  ...
  try {
    ...
    await стр.click('#кнопка-игра-дурак');
    await спать(300);
    await стр.click('#лобби-друзья');
    await спать(500);
    проверить(await видимыйЭкран(стр) === 'экран-друга', '«Пригласить друзей» в лобби открыл экран друга');
    await стр.click('#кнопка-друг-назад');
    await спать(300);
    проверить(await видимыйЭкран(стр) === 'экран-лобби', 'сценарий ревьюера: «Назад» с экрана друга — лобби дурака, а не витрина');
    await стр.click('#экран-лобби [data-путь="назад"]');
    await спать(300);
    ...
    if (!естьСкрипт) {
      ...
      await тг.click('#лобби-друзья');
      await спать(500);
      await нажатьВTelegram('back_button_pressed');
      await спать(400);
      проверить((await сост()).экран === 'экран-лобби', 'сценарий ревьюера: стрелка Telegram с экрана друга — в лобби дурака');
      await нажатьВTelegram('back_button_pressed');
      await спать(400);
      с = await сост();
