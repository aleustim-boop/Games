...
/* Ломающий запуск: node tests/витрина-пальцем-в-браузере.js --сломать-назад-с-рейтинга
   ...
   что-либо делать. Показывает, что оба места, где стенд нажимает эту
   кнопку напрямую, обязаны покраснеть, если обработчик пропадёт. */
...
function поднятьРаздачу(подменаРейтингЭкран) {
...
(async function () {
  ...
  const раздача = await поднятьРаздачу(СЛОМАТЬ_НАЗАД_С_РЕЙТИНГА ? испорченныйТекстРейтингЭкран() : null);
