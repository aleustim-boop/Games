...
async function пройти() {
  ...
  /* В живом файле знакомств владельца могут лежать настоящие записи (файл
     сберегается и возвращается) — считаем, что прибавилось ровно одно,
     а не что оно одно на весь сервер (11 сентября там оказалась чужая запись). */
