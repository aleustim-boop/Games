...
/* Данные уводим в свою временную папку ДО подключения сервера: в
   server/данные лежат настоящие знакомства владельца, а знакомые.сколько
   Знакомств() ниже читает файл при первом обращении — без увода это
   было бы чтением боевых записей в память проверки. Раньше здесь стоял
   снимок-и-возврат (tests/сберечь-данные.js): он подстраховывал на
   выходе, но до этого момента всё равно читал настоящий файл. Путь
   задаётся переменной ДАННЫЕ_ИГРЫ, которую понимает server/папка-данные.js. */
const СВОЯ_ПАПКА = fs.mkdtempSync(path.join(os.tmpdir(), 'проверка-сеть-знакомые-'));
...
async function пройти() {
  ...
  проверить('знакомство на сервере есть', знакомые.сколькоЗнакомств() === былоЗнакомств + 1);
