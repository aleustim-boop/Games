...
(async () => {
  ...
  console.log('\n=== 5. Стрелка «Назад» Telegram закрывает правила первой ===');
  {
    ...
    /* Шагов стало три: у шашек появился свой экран «с кем играть», и стрелка
       отматывает по одному — правила → доска → выбор соперника → витрина игр. */
    await страница.evaluate(() => window.ПоддельныйТелеграм.нажатьНазад());
    await страница.waitForTimeout(400);
    проверить(await страница.evaluate(() => window.ШашкиРежимы.открыты()),
      'второе нажатие увело с доски на экран «с кем играть»');
    await страница.evaluate(() => window.ПоддельныйТелеграм.нажатьНазад());
    await страница.waitForTimeout(500);
    проверить(/index\.html$/.test(страница.url()), 'третье нажатие увело на index.html: ' + страница.url());
    await окно.close();
  }
