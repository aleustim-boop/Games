...
function блокДолгаяОчередь(данные) {
  ...
  // Дашборду коротко: первые строки без длинной «Заметки штаба».
  const ПОКАЗАТЬ = 8;
  const показанные = д.список.slice(0, ПОКАЗАТЬ);

  let строки = '';
  показанные.forEach(function (з) {
    строки += '<li>'
      + '<span class="метка-версии">' + э(з.уровень) + '</span>'
      + '<span class="что">' + э(з.что) + '</span>'
      + '</li>';
  });
  ...
  <section>
    ...
    <p class="пояснение">Не срочное из штаб/очередь.md: «потом» — нужное, но никто не ждёт; «задел» — не начинать, пока не спросят.</p>
