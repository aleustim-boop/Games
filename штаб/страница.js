...
function шапка(данные) {
  ...
  const списокПравился = данные.задачи.правился
    ? '<br>список задач правился: <b>' + э(сбор.часыМинуты(new Date(данные.задачи.правился))) + '</b>'
    : '';
...
function сводка(данные) {
  ...
  <div class="сводка">
    ...
    <div class="число число--янтарь"><strong>${данные.задачи.очередь.length}</strong><span>в очереди</span></div>
...
function блокОчередь(данные) {
  ...
  очередь.forEach(function (з, номер) {
    ...
    строки += '<li' + классСтроки + '>'
      ...
      + '<span class="задача">' + э(з.что) + заметка + '</span>'
  ...
  <section>
    ...
    <p class="пояснение">Четыре уровня. <b>Горит</b> — игрок видит поломку, берём немедленно. <b>Скоро</b> — вы ждёте, спросите на днях. <b>Потом</b> — нужное, но никто не ждёт. <b>Задел</b> — пригодится, когда вырастем.</p>
