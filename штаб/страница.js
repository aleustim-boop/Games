...
const ОФОРМЛЕНИЕ = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Правка «Долгая очередь»: маленькая форма при каждой строке — сменить
...
function сводка(данные) {
  ...
  // Наверху страница обязана показывать ВЕСЬ бэклог, не только короткую
  // таблицу «Очередь» из ЗАДАЧИ.md (горит/скоро) — иначе владелец, увидев
  // маленькое число, решает, что список пуст, хотя долгая очередь полна.
  const долгаяОчередьВсего = (данные.долгаяОчередь && данные.долгаяОчередь.есть) ? данные.долгаяОчередь.всего : 0;
  const вОчередиВсего = данные.задачи.очередь.length + долгаяОчередьВсего;
...
/**
 * Долгая очередь: штаб/очередь.md — важность «потом»/«задел», которую
 * в ЗАДАЧИ.md не видно (там только «горит»/«скоро»). Без этого блока
 * раздел «Очередь» на дашборде выглядел пустым, хотя работа по хвосту
 * реально идёт — счётчики и первые строки показывают, что он не пуст.
 */
function блокДолгаяОчередь(данные) {
  const д = данные.долгаяОчередь;

  if (!д || !д.есть) {
    return `
  <section>
    <h2>Долгая очередь (потом/задел)</h2>
    <p class="пояснение">Список показать нечем: ${э((д && д.беда) || 'штаб/очередь.md не найден')}.</p>
  </section>`;
  }

  // Владелец правит очередь прямо на странице — значит, видеть и редактировать
  // он должен ВЕСЬ список, а не первые строки: иначе половина задач для него
  // как будто не существует. Длина не беда — это простой список, не таблица.
  const показанные = д.список;
  ...
  let строки = '';
  показанные.forEach(function (з) {
    строки += '<li>'
      + '<span class="метка-версии">' + э(з.уровень) + '</span>'
      + '<span class="что">' + э(з.что) + формаЗаписи(з) + '</span>'
      + '</li>';
  });
  if (!строки) строки = '<li><span class="метка-версии">—</span><span class="что">очередь пуста</span></li>';

  const остаток = д.всего - показанные.length;
  const ещё = остаток > 0
    ? '<p class="пояснение">И ещё ' + счётСловом(остаток, 'задача', 'задачи', 'задач') + ' в файле штаб/очередь.md.</p>'
    : '';

  const счёт = д.всего
    ? счётСловом(д.всего, 'задача', 'задачи', 'задач') + ' · потом ' + д.потом + ' · задел ' + д.задел
    : 'пусто';

  // Форма «Добавить задачу» — новая запись сразу пишется в штаб/списки.json,
  // важность по умолчанию «задел» (не начинать, пока не спросят).
  const формаДобавить = '<form class="форма-письма форма-очереди-добавить" method="POST" action="/очередь-добавить">'
    + '<label class="форма-письма__подпись" for="очередь-новое-что">Добавить задачу в долгую очередь</label>'
  ...
  return `
  <section>
    <h2>Долгая очередь (потом/задел) <span class="счёт">${э(счёт)}</span></h2>
    <p class="пояснение">Не срочное из штаб/списки.json: «потом» — нужное, но никто не ждёт; «задел» — не начинать, пока не спросят.</p>
