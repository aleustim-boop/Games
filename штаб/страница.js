...
const ОФОРМЛЕНИЕ = `
  ...
  * { box-sizing: border-box; }
  ...
  /* Одна строка очереди: номер, метка важности, текст задачи и под ним —
     формы правки (порядок, важность, удаление). Не ниже 44 точек по высоте
     полосы кнопок: по ним попадают пальцем. */
  .очередь li {
    display: flex; flex-wrap: wrap; align-items: baseline; gap: 6px 12px;
  ...
  .задача { font-size: 16px; flex: 1 1 220px; }
  .задача em { display: block; font-style: normal; font-size: 14px; color: var(--muted); margin-top: 3px; }
  /* Формы правки строки очереди: порядок (▲▼), важность, удаление — все
     без своего JS, обычными HTML-формами, в один ряд под текстом задачи. */
  .форма-ряд { display: flex; align-items: center; gap: 8px; flex-wrap: wrap; margin-top: 6px; flex-basis: 100%; }
  .форма-порядок { display: inline-flex; margin: 0; }
  .форма-порядок__кнопка { padding: 4px 10px; line-height: 1; }
  .форма-порядок__кнопка:disabled { opacity: .3; cursor: default; }
  .метка {
  ...
  /* Правка очереди: маленькая форма при каждой строке — сменить
     важность или удалить, без своего JS, обычной HTML-формой. */
  .форма-очереди {
    display: flex; align-items: center; gap: 6px; margin-top: 6px; flex-wrap: wrap;
  }
  .форма-очереди__важность {
    font-size: 13.5px; padding: 4px 6px; border-radius: 8px;
    border: 1px solid var(--line); background: var(--surface); color: var(--ink);
  }
  .форма-очереди__кнопка {
    font-size: 13px; padding: 4px 10px; border-radius: 8px; border: 1px solid var(--line);
    background: var(--surface); color: var(--ink); cursor: pointer;
  }
  .форма-очереди__кнопка--убрать { color: var(--chervi); border-color: var(--chervi-soft); }
  .форма-очереди-добавить { margin-top: 14px; }
  .форма-очереди-добавить__поле { min-height: 56px; }
  .форма-очереди-добавить__низ { display: flex; align-items: center; gap: 10px; }
  .лента .не-выложено {
...
function сводка(данные) {
  ...
  // Один источник очереди — штаб/списки.json целиком (все важности сразу),
  // короткая таблица «Очередь» из ЗАДАЧИ.md больше отдельно не считается.
  const вОчередиВсего = (данные.очередьЕдиная && данные.очередьЕдиная.есть) ? данные.очередьЕдиная.всего : 0;
  ...
  <div class="сводка">
    ...
    <div class="число"><strong>${вОчередиВсего}</strong><span>в очереди</span></div>
...
/**
 * Очередь — один список на всю страницу: весь штаб/списки.json, любая
 * важность (горит/скоро/потом/задел). Раньше было два раздела с двумя
 * разными источниками и двумя разными смыслами важности; владелец решил,
 * что ему нужен один список, а порядок записей в нём — это и есть
 * приоритет: верхняя строка делается первой. Кнопки ▲▼ у каждой строки
 * меняют её местами с соседней прямо в штаб/списки.json (без своего JS,
 * обычной HTML-формой, как важность и удаление).
 */
function блокОчередь(данные) {
  const д = данные.очередьЕдиная;

  if (!д || !д.есть) {
    return `
  <section>
    <h2>Очередь</h2>
    <p class="пояснение">Список показать нечем: ${э((д && д.беда) || 'штаб/списки.json не найден')}.</p>
  </section>`;
  }

  const список = д.список; // порядок в массиве — это и есть приоритет, не трогаем сами
  const УРОВНИ = ['горит', 'скоро', 'потом', 'задел'];

  /** Строка формы у одной записи: сменить важность или удалить — без своего JS. */
  function формаЗаписи(з) {
    const параметры = УРОВНИ.map(function (у) {
      return '<option value="' + у + '"' + (у === з.уровень ? ' selected' : '') + '>' + у + '</option>';
    }).join('');
    return '<form class="форма-очереди" method="POST" action="/очередь-изменить">'
      + '<input type="hidden" name="индекс" value="' + э(з.индекс) + '">'
      + '<select class="форма-очереди__важность" name="важность">' + параметры + '</select>'
      + '<button class="форма-очереди__кнопка" type="submit">Сохранить</button>'
      + '<button class="форма-очереди__кнопка форма-очереди__кнопка--убрать" type="submit" formaction="/очередь-удалить">Удалить</button>'
      + '</form>';
  }

  /** Пара кнопок «выше»/«ниже»: меняют запись местами с соседней — тоже без своего JS. */
  function формаПорядка(з, первая, последняя) {
    return '<form class="форма-порядок" method="POST" action="/очередь-выше">'
      + '<input type="hidden" name="индекс" value="' + э(з.индекс) + '">'
      + '<button class="форма-очереди__кнопка форма-порядок__кнопка" type="submit"' + (первая ? ' disabled' : '') + ' title="Поднять выше">▲</button>'
      + '</form>'
      + '<form class="форма-порядок" method="POST" action="/очередь-ниже">'
      + '<input type="hidden" name="индекс" value="' + э(з.индекс) + '">'
      + '<button class="форма-очереди__кнопка форма-порядок__кнопка" type="submit"' + (последняя ? ' disabled' : '') + ' title="Опустить ниже">▼</button>'
      + '</form>';
  }

  let строки = '';
  список.forEach(function (з, место) {
    const классСтроки = (з.уровень === 'горит' || з.уровень === 'скоро' || з.уровень === 'потом') ? ' class="' + з.уровень + '"' : '';
    строки += '<li' + классСтроки + '>'
      + '<span class="номер">' + String(место + 1).padStart(2, '0') + '</span>'
      + '<span class="метка метка--' + з.уровень + '">' + э(з.уровень) + '</span>'
      + '<span class="задача">' + э(з.что) + '<div class="форма-ряд">' + формаПорядка(з, место === 0, место === список.length - 1) + формаЗаписи(з) + '</div></span>'
      + '</li>';
  });
  if (!строки) строки = '<li><span class="номер">—</span><span class="задача">очередь пуста</span></li>';
  ...
  const пусто = !список.length
    ? '<p class="пояснение">Очередь пуста. Если вы ждёте чего-то ещё — скажите, этого просто нет в списке.</p>'
    : '';

  const счёт = д.всего
    ? счётСловом(д.всего, 'задача', 'задачи', 'задач') + ' · горит ' + д.горит + ' · скоро ' + д.скоро + ' · потом ' + д.потом + ' · задел ' + д.задел
    : 'пусто';

  // Форма «Добавить задачу» — новая запись сразу пишется в штаб/списки.json,
  // важность по умолчанию «задел» (не начинать, пока не спросят), в конец списка.
  const формаДобавить = '<form class="форма-письма форма-очереди-добавить" method="POST" action="/очередь-добавить">'
    + '<label class="форма-письма__подпись" for="очередь-новое-что">Добавить задачу в очередь</label>'
    + '<textarea class="форма-письма__поле форма-очереди-добавить__поле" id="очередь-новое-что" name="что" rows="2" maxlength="300" '
    + 'placeholder="Что сделать, своими словами"></textarea>'
    + '<div class="форма-очереди-добавить__низ">'
    + '<select class="форма-очереди__важность" name="важность">'
    + УРОВНИ.map(function (у) { return '<option value="' + у + '"' + (у === 'задел' ? ' selected' : '') + '>' + у + '</option>'; }).join('')
    + '</select>'
    + '<button class="форма-письма__кнопка" type="submit">Добавить</button>'
    + '</div></form>';
  ...
  <section>
    ...
    <details class="очередь-легенда" data-ключ="уровни-легенда">
      ...
      <p class="пояснение"><b>Горит</b> — игрок видит поломку, берём немедленно. <b>Скоро</b> — вы ждёте, спросите на днях. <b>Потом</b> — нужное, но никто не ждёт. <b>Задел</b> — пригодится, когда вырастем. Порядок в списке — и есть приоритет: верхняя строка делается первой, стрелками ▲▼ можно её передвинуть.</p>
    </details>
    ${тревога}${пусто}
