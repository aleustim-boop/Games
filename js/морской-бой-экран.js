...
(function () {
  ...
  function узелВДрузья() {
    ...
    if (!узел) { узел = document.createElement('div'); узел.id = 'гнездо-друзей'; узел.className = 'подпись'; $('море-результат').appendChild(узел); }
    return узел;
  }
  /** Партия с ботом (или новая сетевая) — блока «В друзья» от прошлого боя здесь быть не должно. */
  ...
  function итог() {
    ...
    if (!итогПоказан) {
      итогПоказан = true; открыть('море-результат');
      // «В друзья» — только по сети, один запрос на этот бой.
      if (поСети && typeof window.Сеть?.кнопкаВДрузья === 'function') window.Сеть.кнопкаВДрузья(узелВДрузья());
    }
  }
  function рисовать() {
    clearTimeout(timer); timer = null;
    ...
    $('море-очередь').classList.toggle('ждём', !вид.вашХод || busy || вид.фаза === 'конец');
