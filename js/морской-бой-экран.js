...
(function () {
  ...
  function сохранить() {
    try { if (запись && !поСети) { запись.черновик = черновик; localStorage.setItem(KEY, JSON.stringify(запись)); } localStorage.setItem(PREF, JSON.stringify(настройки)); }
  ...
  const лист = window.ЛистЕщё.подключить({ двери: { вМеню: { действие: назад }, какИграть: { действие: () => открыть('море-справка') }, рекорды: { действие: история }, звук: { состояние: () => настройки.звук ? 'вкл' : 'выкл', действие: () => { настройки.звук = !настройки.звук; сохранить(); } } } });
  ...
  window.ИграПоСети = {
    ...
    показатьВид(v) {
      ...
      if (fresh) { черновик = []; цель = null; итогПоказан = false; последний = ''; $('море-результат').close(); }
