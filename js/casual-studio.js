...
window.CasualStudio=(()=>{
 ...
 const assets={match3:['jewels-room','tray','../play/gems','../play/felt'],mahjong:['mahjong-room','mahjong-tile','../play/felt'],klondike:['card-room','card-back','../play/felt'],spider:['card-room','card-back','../play/felt'],freecell:['card-room','card-back','../play/felt'],mines:['mines-room','brass-frame','materials'],blocks:['jewels-room','brass-frame','materials','blocks'],wordsearch:['word-room','brass-frame','materials'],fifteen:['word-room','wood-frame','materials','../play/felt'],snake:['mahjong-room','brass-frame','materials','snake-pieces']};
 function prepare(id){return Promise.all((assets[id]||[]).map(name=>new Promise((resolve,reject)=>{const img=new Image(),timer=setTimeout(()=>reject(new Error('Image timeout')),15000);img.src='img/casual/studio/'+name+'.webp';img.decode().then(()=>{clearTimeout(timer);resolve();},()=>{clearTimeout(timer);reject(new Error('Image unavailable'));});})));}
