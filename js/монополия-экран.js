...
(function () {
  ...
  // Нижняя полоса «Игры · Рейтинг · Профиль» — общий вид, как у прочих
  // настольных игр (штаб/план-рейтинг-во-всех-играх.md, Э2). У монополии
  // нет своего js/лобби-настольных.js, поэтому разметку пустой <nav>
  // заполняем прямо здесь: набор "лобби-деберц" даёт те же подписи
  // и значки, что у домино и деберца, а свои id и обработчики вешаем сами.
  (function () {
    const полоса = document.querySelector("#экран-лобби .нижние-вкладки");
    if (!полоса || !window.НижниеВкладки) return;
    const идВкладок = [
      "mono-вкладка-игры",
      "mono-вкладка-рейтинг",
      "mono-вкладка-профиль",
    ];
    полоса.innerHTML = window.НижниеВкладки.вкладкиНабора("лобби-деберц")
      .map((вкладка, i) =>
        window.НижниеВкладки.кнопкаHTML({ ...вкладка, id: идВкладок[i] }),
      )
      .join("");
    $(идВкладок[0]).onclick = () => (location.href = "index.html");
    $(идВкладок[1]).onclick = () => window.ЭкранРейтинга.открыть("монополия");
    $(идВкладок[2]).onclick = () => window.ЭкранПрофиля.открыть("монополия");
  })();
  window.ИграПоСети = {
    начать() {
      clearTimeout(timer);
